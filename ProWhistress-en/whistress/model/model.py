# model.py 该文件实现了WhiStress模型的主结构，包含Whisper骨干网络、重音检测自定义头、模型保存/加载、推理等功能
...
class WhiStress(PreTrainedModel):
    ...
    def __init__(
        ...
        enc_heads = getattr(config, "encoder_attention_heads", 8)
    
        self.stress_encoder = StressEncoder(d_model=input_dim, num_heads=enc_heads, num_layers=stress_encoder_layers)
    ...
    def train(self, mode: Optional[bool] = True):
        ...
        for param in self.stress_encoder.parameters():
            param.requires_grad = not self.freeze_stress_encoder
        for param in self.dec_to_ctx.parameters():
            param.requires_grad = True
        for param in self.enc_to_ctx.parameters():
            param.requires_grad = True
        for param in self.ctx_to_model.parameters():
            param.requires_grad = True
        for param in self.audio_feature_extractor.parameters():
            param.requires_grad = True
        ...
        if self.freeze_stress_encoder:
            ...
            print("🔒 StressEncoder is frozen (Linear Probing mode)")
        else:
