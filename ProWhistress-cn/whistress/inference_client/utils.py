...
LOCAL_WHISPER_MODEL = pathlib.Path(__file__).parent.parent / "whisper_model" / "whisper" / "whisper-small"


def get_loaded_model(device="cuda", model_path=None):
    """
    Load the WhiStress (Chinese) model.
    If model_path is provided, weights and metadata are loaded from that path;
    otherwise falls back to the default weights in whistress/weights.
    """
    weights_dir = pathlib.Path(model_path) if model_path else PATH_TO_WEIGHTS

    # Read hyperparameters from metadata if available
    d_ctx = 256
    ...
    # Prefer local whisper model; fall back to HuggingFace
    whisper_backbone = str(LOCAL_WHISPER_MODEL) if LOCAL_WHISPER_MODEL.exists() else "openai/whisper-small"

    whisper_config = WhisperConfig()
    whistress_model = WhiStress(
        whisper_config,
        layer_for_head=layer_for_head,
        whisper_backbone_name=whisper_backbone,
        d_ctx=d_ctx,
    ...
    whistress_model.load_model(str(weights_dir))
