# 该文件实现了WhiStress模型的自定义训练器，支持token级和word级评估、模型保存等
...
class WhiStressTrainer(Seq2SeqTrainer):
    ...
    def save_final_model(self, output_dir=None, training_args=None):
        ...
        if output_dir is not None:
            ...
            # 获取stress_encoder_layers
            stress_encoder_layers = 2 # 默认值
            if hasattr(self.model, "stress_encoder"):
                # 检查是否是WhisperEncoder (新版)
                if hasattr(self.model.stress_encoder, "layers"):
                    stress_encoder_layers = len(self.model.stress_encoder.layers)
                # 检查是否是旧版StressEncoder (包含encoder属性)
                elif hasattr(self.model.stress_encoder, "encoder") and hasattr(self.model.stress_encoder.encoder, "layers"):
                    stress_encoder_layers = len(self.model.stress_encoder.encoder.layers)
                # 检查是否是旧版StressEncoder (包含encoder属性且是TransformerEncoder)
                elif hasattr(self.model.stress_encoder, "encoder") and hasattr(self.model.stress_encoder.encoder, "num_layers"):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval", dataset_name=''):
        ...
        # 触发on_evaluate回调，确保EarlyStoppingCallback和CustomCallback能接收到指标
    ...
    def evaluate_at_word_level(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval", dataset_name=''):
        ...
        # 检查是否存在 map_dict 列
        map_dict_cols = [elem for elem in eval_dataset.column_names if "map_dict" in elem]
        if not map_dict_cols:
            print(f"⚠️ 警告: 数据集中未找到 'map_dict' 列，跳过字级别评估。")
            return {}
        map_dict_key = map_dict_cols[-1]
        ...
        for batch in tqdm(eval_dataloader):
            ...
            # map_dict_key 已经在循环外获取
            for i in range(labels_head.shape[0]):
                j_start = 1
                labels_head_by_words = [-100]
                generated_ids_by_words = [-100]
                # 解析JSON格式的map_dict
                map_dict_str = eval_dataset[int(batch_samples[i])][map_dict_key]
                if isinstance(map_dict_str, str):
                    map_dict = json.loads(map_dict_str)
                else:
                    map_dict = map_dict_str
                ...
                if isinstance(map_dict, dict):
                    if "values" in map_dict:
                        ...
                        # 按键的整数值排序提取值
                        try:
                            sorted_keys = sorted(map_dict.keys(), key=lambda x: int(x))
                            map_values = [map_dict[k] for k in sorted_keys]
                        except ValueError:
                            # 如果键不是整数，尝试直接取values（虽然顺序可能不对，但这是最后的尝试）
                            print(f"⚠️ map_dict 键格式异常，无法排序: {list(map_dict.keys())[:5]}...")
                            map_values = list(map_dict.values())
                elif isinstance(map_dict, list):
                    map_values = map_dict
                else:
                    print(f"⚠️ map_dict 格式未知: {type(map_dict)}")
                    map_values = []
                ...
                for val in map_values:
                    ...
                    while (not np.array_equal(whisper_labels[i][j_start:j_end].cpu().numpy(), np.array(val)) and \
                        j_end < len(whisper_labels[i]) and not whisper_labels[i][j_end].item() == 50256 and not len(val) == 0):
                ...
                if j_end < len(whisper_labels[i]) and whisper_labels[i][j_end].item() != 50256: # 50256 relates to the choice of the backbone as whisper's english model
    ...
    def evaluate_at_char_level(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval", dataset_name=''):
        """
        Evaluate model at Chinese character level on the evaluation dataset.
        
        This method is specifically designed for Chinese text evaluation where each 
        character is meaningful. It aggregates token-level predictions to the character 
        level before computing metrics.
        
        A character is considered emphasized if any of its tokens are predicted as emphasized.
        ...
        Returns:
            Dictionary of character-level evaluation metrics
        """
        eval_dataloader = self.get_eval_dataloader(eval_dataset)
        self.model.eval()
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating at character level"):
            # Extract input features and labels
            input_features = batch["input_features"]
            labels_keys = [elem for i, elem in enumerate(batch.keys()) if "labels" in elem and not "labels_head" in elem][-1]
            whisper_labels = batch[labels_keys]
            ...
            all_labels_head_by_chars = []
            all_generated_ids_by_chars = []
            batch_samples = torch.where(torch.tensor(eval_dataset['sentence_index']).cpu() == batch['sentence_index'].unsqueeze(1).cpu())[1].numpy()
            
            # 检查是否存在map_dict列，如果不存在则跳过字符级别评估
            map_dict_keys = [elem for i, elem in enumerate(eval_dataset.column_names) if "map_dict" in elem]
            if not map_dict_keys:
                print(f"Warning: No map_dict column found in dataset. Skipping character-level evaluation.")
                print(f"Available columns: {eval_dataset.column_names}")
                # 返回token级别的评估结果作为替代
                return self.evaluate(eval_dataset=eval_dataset, ignore_keys=ignore_keys, metric_key_prefix=metric_key_prefix)
            
            map_dict_key = map_dict_keys[-1]
            for i in range(labels_head.shape[0]):
                j_start = 1  # Skip the first special token
                labels_head_by_chars = [-100]  # Start with special token
                generated_ids_by_chars = [-100]
                
                # Process each character's token mapping
                # Parse JSON string to dictionary
                map_dict_str = eval_dataset[int(batch_samples[i])][map_dict_key]
                if isinstance(map_dict_str, str):
                    map_dict_data = json.loads(map_dict_str)
                else:
                    map_dict_data = map_dict_str
                    
                # 兼容不同的map_dict格式
                if isinstance(map_dict_data, dict):
                    if "values" in map_dict_data:
                        map_values = map_dict_data["values"]
                    else:
                        # 假设键是字符串索引 "0", "1", ...
                        # 按键的整数值排序提取值
                        try:
                            sorted_keys = sorted(map_dict_data.keys(), key=lambda x: int(x))
                            map_values = [map_dict_data[k] for k in sorted_keys]
                        except ValueError:
                            # 如果键不是整数，尝试直接取values
                            print(f"⚠️ map_dict 键格式异常，无法排序: {list(map_dict_data.keys())[:5]}...")
                            map_values = list(map_dict_data.values())
                elif isinstance(map_dict_data, list):
                    map_values = map_dict_data
                else:
                    print(f"⚠️ map_dict 格式未知: {type(map_dict_data)}")
                    map_values = []

                for char_tokens in map_values:
                    if len(char_tokens) == 0:
                        # Empty token list, skip this character
                        continue
                    
                    j_end = j_start + len(char_tokens)
                    
                    # Ensure we don't exceed the sequence length
                    if j_end > len(whisper_labels[i]):
                        break
                    
                    # Check if any token in this character is labeled as emphasized
                    char_has_emphasis_label = torch.any(labels_head[i][j_start:j_end] == 1).item()
                    char_has_emphasis_pred = torch.any(generated_ids[i][j_start:j_end] == 1).item()
                    
                    labels_head_by_chars.append(1 if char_has_emphasis_label else 0)
                    generated_ids_by_chars.append(1 if char_has_emphasis_pred else 0)
                    
                    j_start = j_end
                
                # Pad sequences to the same length
                labels_head_by_chars_padded = self._pad_tensors_to_max_len(
                    torch.tensor(labels_head_by_chars).unsqueeze(0), max_length=self.args.generation_max_length
                )
                generated_ids_by_chars_padded = self._pad_tensors_to_max_len(
                    torch.tensor(generated_ids_by_chars).unsqueeze(0), max_length=self.args.generation_max_length
                )
                
                all_generated_ids_by_chars.append(generated_ids_by_chars_padded.squeeze(0))
                all_labels_head_by_chars.append(labels_head_by_chars_padded.squeeze(0))
                
            padded_labels_by_chars = torch.stack(all_labels_head_by_chars)
            padded_preds_by_chars = torch.stack(all_generated_ids_by_chars)
            
            all_preds_by_chars.append(padded_preds_by_chars.cpu().numpy())
            all_labels_by_chars.append(padded_labels_by_chars.cpu().numpy())

        # Concatenate all batches
        all_preds_by_chars = np.concatenate(all_preds_by_chars, axis=0)
        all_labels_by_chars = np.concatenate(all_labels_by_chars, axis=0)

        # Compute metrics
        outputs_metrics = {}
        if self.compute_metrics is not None:
            metrics = self.compute_metrics(
                {"predictions": all_preds_by_chars, "label_ids": all_labels_by_chars}
            )
            for key, value in metrics.items():
                key = f"{metric_key_prefix}_char_{key}"
                if isinstance(value, np.ndarray):
                    outputs_metrics[key] = value.tolist()
                else:
                    outputs_metrics[key] = value
                    
        with open(os.path.join(self.args.output_dir, "log_eval_char_level.txt"), "a") as file:
            json.dump(f'Evaluate at CHARACTER LEVEL {dataset_name}:', file)
            json.dump(outputs_metrics, file)
        self.log(outputs_metrics)
        return outputs_metrics
